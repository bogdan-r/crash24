  ...
  create: function (req, res) {
    ...
    User.create(userParams).exec(function (err, user) {
      ...
      Email.send({
        ...
        html: 'Для подтверждения регистрации перейдите по ссылке ' +
  ...
  profile: function (req, res) {
    ...
    q.fcall(function () {
      ...
      for (var i = 0; i < user.messages.length; i++) {
        ...
        if (user.messages[i].user.id == user.id) {
          user.messages[i].isSentMessage = true
        }
  ...
  getVerificateTokenByEmail: function (req, res) {
    ...
    User.findOne(req.user.id).exec(function (err, user) {
      ...
      Email.send({
        ...
        subject: 'Подтверждение регистрации',
        html: 'Вы повторно запросили ссылку для подтверждения регистрации: ' +
